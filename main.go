package main

import (
	"bytes"
	"log"
	// "fmt"
	// "net/http"
	"encoding/json"
	"time"

	"go.opentelemetry.io/collector/pdata/pcommon"
	"go.opentelemetry.io/collector/pdata/plog"
	"go.opentelemetry.io/collector/pdata/plog/plogotlp"
)

var (
	logTimestamp = pcommon.NewTimestampFromTime(time.Date(2020, 2, 11, 20, 26, 13, 789, time.UTC))
)

func initResourceAttributes1(dest pcommon.Map) {
	dest.Clear()
	dest.PutStr("resource-attr", "resource-attr-val-1")
}

func initResource(r pcommon.Resource) {
	initResourceAttributes1(r.Attributes())
}

func GenerateLogs(count int) plog.Logs {
	ld := plog.NewLogs()
	initResource(ld.ResourceLogs().AppendEmpty().Resource())
	logs := ld.ResourceLogs().At(0).ScopeLogs().AppendEmpty().LogRecords()
	logs.EnsureCapacity(count)
	for i := 0; i < count; i++ {
		switch i % 2 {
		case 0:
			fillLogOne(logs.AppendEmpty())
		case 1:
			fillLogTwo(logs.AppendEmpty())
		}
	}
	return ld
}

func fillLogOne(log plog.LogRecord) {
	log.SetTimestamp(logTimestamp)
	log.SetDroppedAttributesCount(1)
	log.SetSeverityNumber(plog.SeverityNumberInfo)
	log.SetSeverityText("Info")
	log.SetSpanID([8]byte{0x01, 0x02, 0x04, 0x08})
	log.SetTraceID([16]byte{0x08, 0x04, 0x02, 0x01})

	attrs := log.Attributes()
	attrs.PutStr("app", "server")
	attrs.PutInt("instance_num", 1)

	// log.Body().SetStr("This is a log message")
	log.Body().SetEmptySlice().AppendEmpty().SetEmptyMap().FromRaw(map[string]interface{}{"k": "v"})
	// log.Body().SetEmptyMap().FromRaw(map[string]interface{}{"k": []string{"v","a"}})
}

func fillLogTwo(log plog.LogRecord) {
	log.SetTimestamp(logTimestamp)
	log.SetDroppedAttributesCount(1)
	log.SetSeverityNumber(plog.SeverityNumberInfo)
	log.SetSeverityText("Info")

	attrs := log.Attributes()
	attrs.PutStr("customer", "acme")
	attrs.PutStr("env", "dev")

	log.Body().SetStr("something happened")
}
func main(){
	res := GenerateLogs(1)	
	tr := plogotlp.NewExportRequestFromLogs(res)
	body, _ := tr.MarshalJSON()
	var prettyJSON bytes.Buffer
  error := json.Indent(&prettyJSON, body, "", "\t")
	if error != nil {
        log.Println("JSON parse error: ", error)
			}
	log.Println("CSP Violation:", string(prettyJSON.Bytes()))
	// request, _ := tr.MarshalProto()
	// resp, _ := http.Post("http://0.0.0.0:4318/", "application/x-protobuf",
        // bytes.NewBuffer(request))
	// fmt.Printf("%+v", resp)
}
